    ...
    _createClass(FormItem, [{
            ...
            if (beforeSubmit && validate && validate.length) {
                ...
                validate.map(function (item) {
                    if (!disabled && item.trigger === "submit") {
                        disabled = _this2.validator(item, data, "submit");
                    }
                });
        ...
        value: function itemRender() {
            var _props3 = this.props,
                ...
                config = _objectWithoutProperties(_props3, ['on', 'off', 'tips', 'col', 'requiredMark', 'name', 'data', 'colon', 'component', 'className', 'dataFormat', 'content', 'value', 'type', 'onBlur', 'beforeSubmit', 'onChange', 'children', 'options', 'validate', 'validateType', 'validator', 'labelWidth']);
...
FormItem.propTypes = {
    ...
    type: _propTypes2['default'].oneOf(['text', 'color', 'editor', 'static', 'datetime', 'number', 'component', 'password', 'textarea', 'select', 'checkbox', 'radio', 'switch', 'upload', 'radiogroup', 'checkgroup', 'checkboxgroup'])
};
