    ...
    _createClass(FormItem, [{
            ...
            if (validate && validate.length) {
                validate.map(function (item) {
                    if (!disabled && item.trigger === "change") {
                        disabled = _this4.validator(item, value === undefined ? data : value);
                    }
                });
            }
            if (!disabled && this.msg_str) {
        ...
        value: function itemRender() {
            var _props3 = this.props,
                ...
                config = _objectWithoutProperties(_props3, ['on', 'off', 'tips', 'col', 'name', 'data', 'component', 'className', 'content', 'value', 'type', 'onBlur', 'beforeSubmit', 'onChange', 'children', 'options', 'validate', 'validateType', 'validator', 'labelWidth']);
...
FormItem.propTypes = {
    ...
    type: _propTypes2['default'].oneOf(['text', 'color', 'static', 'component', 'password', 'textarea', 'select', 'checkbox', 'radio', 'switch', 'uploader', 'radiogroup', 'checkgroup'])
};
