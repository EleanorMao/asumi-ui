...
var Form = function (_Component) {
    ...
    _createClass(Form, [{
        ...
        value: function validator(data, options) {
            ...
            options.forEach(function (item) {
                ...
                if (isRequired(item)) {
                    ...
                    _this2.requiredMap[item.name] = validateValue;
                    if (!disabled) disabled = validateValue;
                } else {
                    _this2.requiredMap[item.name] = false;
                }
                names.push(item.name);
            });
            this.names.forEach(function (name) {
                if (!~names.indexOf(name)) {
                    delete _this2.disabledMap[name];
                    delete _this2.requiredMap[name];
                }
            });
            this.names = names;
            disabled = disabled || !!(~(0, _util.getValues)(this.disabledMap).indexOf(true) || ~(0, _util.getValues)(this.requiredMap).indexOf(true));
            if (disabled !== this.state.disabled) this.setState({ disabled: disabled });
        }
    }, {
        key: 'handleDisabled',
        value: function handleDisabled(props, _disabled, cancelSubmitPending) {
            ...
            var disabled = !!_disabled || !!(~(0, _util.getValues)(this.disabledMap).indexOf(true) || ~(0, _util.getValues)(this.requiredMap).indexOf(true));
            if (disabled !== this.state.disabled) this.setState({ disabled: disabled });
        ...
        value: function handleBeforeSubmit(_disabled) {
            var loading = this.state.loading;
            var preventMultipleSubmit = this.props.preventMultipleSubmit;
            ...
            return _react2['default'].createElement(
                ...
                options.map(function (props, index) {
                    return _react2['default'].createElement(_formItem2['default'], _extends({
                        onChange: _this4.handleChange.bind(_this4, props),
                        ...
                        formValidator: _this4.handleDisabled.bind(_this4)
                    }));
                }),
...
Form.propTypes = {
    ...
    options: _propTypes2['default'].arrayOf(_propTypes2['default'].shape({
        ...
        type: _propTypes2['default'].oneOf(['text', 'color', 'password', 'datetime', 'number', 'static', 'component', 'textarea', 'select', 'checkbox', 'radio', 'switch', 'upload', 'radiogroup', 'checkgroup', 'checkboxgroup', 'transfer', 'taginput'])
    })),
